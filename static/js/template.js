(() => {
    var e, o = {
        984: (e, o, i) => {
            ...
            const dropzoneAreas = document.querySelectorAll("[data-dropzone-area]");


            dropzoneAreas.forEach(function (dropzoneArea) {
                const dzBtn = dropzoneArea.querySelector("#dz-btn");
                const dzPreviewRow = dropzoneArea.querySelector("#dz-preview-row");
                const ti = new oi.Dropzone(dropzoneArea, {
                    url: "/",
                    paramName: "files",
                    clickable: dzBtn
                ...
                ti.on('sending', function (file, xhr, formData) {
                    const thread = document.querySelector('.main.active');
                    const messageInput = thread.querySelector('.chat_message');
                    const message = messageInput.value;
                    const threadReceiverId = thread.getAttribute('data-receiver-id');
                    const threadSenderId = thread.getAttribute('data-sender-id');
                    const groupId = thread.getAttribute('data-group-id');
                    const threadId = thread.getAttribute('data-thread-id');

                    if (isReplying === true) {
                        formData.append('action', 'reply_file');
                    ...
                    if (threadReceiverId !== null) {
                        formData.append('receiverId', threadReceiverId);
                    }

                    if (threadSenderId !== null) {
                        formData.append('senderId', threadSenderId);
